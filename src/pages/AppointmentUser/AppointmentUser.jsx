...
import { addChoosenAppointment, appointmentData } from '../../services/appointmentSlice';
...
if (loading) {
  return  <div className='dateConfirm'>Cargando tus citas...</div>;
  ...
  <>
    ...
    <div className='yourAppointments'>Todas tus citas</div>
    <Container>
          ...
          return (
            <Col xs={12} sm={12} md={12} lg={12} xl={6} xxl={4} key={appointment.id}>
              <div className='containerInt'>
                <div
                  className= "contTreatment01"
                >
                  <div>
                  <div className='iconAppointment'><i class="bi bi-journal-text"></i></div>
                  </div>
                  <div className='treatmentLine'>
                  <div className='line01'>Hora:</div><div>{Moment(appointment.date).format('DD/MM/YYYY HH:mm:ss')}</div>
                  </div>
                  <div className='treatmentLine'>
                  <div className='line01'>Estado:</div><div> {appointment.status}</div>
                  </div>
                  <div className='treatmentLine'>
                  <div className='line01'>Coste:</div><div> {appointment.Treatment.price}</div>
                  </div>
                  <div className='treatmentLine'>
                  <div className='line01'>Tratamiento:</div> <div> {appointment.Treatment.name}</div>
                  </div>
                  <div className='treatmentLine'>
                  <div className='line01'>Duración:</div> <div> {appointment.Treatment.duration}</div>
                  </div>
